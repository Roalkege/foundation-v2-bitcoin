...
const Client = function (config, socket, id, authorizeFn) {
  ...
  this.considerBan = function (shareValid) {
    ...
    if (totalShares >= _this.config.settings.banning.checkThreshold) {
      if (
        ...
        _this.config.settings.banning.invalidPercent
      ) {
        this.shares = { valid: 0, invalid: 0 };
      } else {
        _this.socket.destroy();
  ...
  this.broadcastMiningJob = function (parameters) {
    ...
    if (activityAgo > _this.config.settings.timeout.connection) {
      const message = `The last submitted share was ${
        (activityAgo / 1000) | 0
      } seconds ago`;
  ...
  // Handle Stratum suggest_difficulty
  this.handleSuggestDifficulty = function (message) {
    ...
    // Check that Difficulty is Valid
    const difficulty = message.params[0];
    this.enqueueDifficulty(difficulty);
    _this.sendJson({
      id: message.id,
      result: true,
      error: null,
  ...
  this.setupClient = function () {
    ...
    _this.socket.on("error", (error) =>
      _this.emit("client.socket.error", error)
    );
